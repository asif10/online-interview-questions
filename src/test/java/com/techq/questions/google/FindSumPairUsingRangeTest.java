 ...
 * 09/05/2020 20:29
 */
class FindSumPairUsingRangeTest {
    ...
    @DisplayName("should find pair of values matching sum using ranges...")
    @MethodSource
    @ParameterizedTest(name = "{0}")
    void shouldFindPositionInArray(String description, boolean expectedResponse, int[] values, int sum) {
        boolean position = new FindSumPairUsingRange().findPairOfSum(values, sum);
